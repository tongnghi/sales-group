...
with 
    raw_tb_4002_accum as (
        select * , 
        '4002'::text as legal,
        '10'::text as curtype,
        ''::text as profit_center
        from {{ ref("tech_stg_excel_qdtek_tb_south_4002") }} a
        where code_pl != 0
    ),
    tb_4002_period as (
        select  period as new_period
        from raw_tb_4002_accum
        group by period
    ),
    tb_4002_raw_full_period as (
        select  _racct,
                new_period
        from tb_4002_period
        cross JOIN raw_tb_4002_accum
